...
import {
  createLicenseUserSchema, loginLicenseUserSchema, registerLicenseUserSchema,
} from '../lib/schemas.ts';
...
export async function registerAuthRoutes(app: FastifyInstance): Promise<void> {
  ...
  async function requireUser(request: FastifyRequest): Promise<AuthedCaller> {
    ...
    return {
      userId: session.user.id, licenseId: session.user.licenseId,
      role: session.user.role, active: session.user.active,
    };
  }
  ...
  app.post('/register', { config: { rateLimit: { max: 10, timeWindow: '1 hour' } } }, async (request, reply) => {
    ...
    const license = await app.prisma.license.findUnique({ where: { keyHash: hashLicenseKey(input.licenseKey, pepper) } });
    if (!license) throw new ApiError('LIC_NOT_FOUND', 404);

    const existingCount = await app.prisma.licenseUser.count({ where: { licenseId: license.id } });
    ...
    const user = await app.prisma.licenseUser.create({
      data: {
        licenseId: license.id, email, displayName: input.displayName,
        role: decision.role, passwordHash: secret.hash, passwordSalt: secret.salt,
      },
    });

    return reply.status(201).send({ id: user.id, email: user.email, role: user.role, displayName: user.displayName });
  });
  ...
  app.post('/login', { config: { rateLimit: { max: 20, timeWindow: '15 minutes' } } }, async (request, reply) => {
    ...
    const email = normalizeEmail(input.email);
    const user = await app.prisma.licenseUser.findUnique({ where: { licenseId_email: { licenseId: license.id, email } } });
    ...
    if (!user || !user.active || !verifySecret(input.password, { hash: user.passwordHash, salt: user.passwordSalt })) {
    ...
    await app.prisma.$transaction([
      app.prisma.licenseUserSession.create({
        data: {
          userId: user.id, tokenHash: sessionHash(raw),
          expiresAt: new Date(Date.now() + SESSION_TTL_HOURS * 3_600_000),
        },
      }),
      app.prisma.licenseUser.update({ where: { id: user.id }, data: { lastLoginAt: new Date() } }),
    ]);

    return reply.send({
      token: raw, expiresInHours: SESSION_TTL_HOURS,
  ...
  app.delete('/session', async (request, reply) => {
    ...
    if (header?.startsWith('Bearer ')) {
      await app.prisma.licenseUserSession.updateMany({
        where: { tokenHash: sessionHash(header.slice('Bearer '.length)), revokedAt: null },
  ...
  app.get('/me', async (request, reply) => {
    ...
    const user = await app.prisma.licenseUser.findUniqueOrThrow({ where: { id: caller.userId } });
    return reply.send({ id: user.id, email: user.email, role: user.role, displayName: user.displayName, licenseId: user.licenseId });
  });
  ...
  app.post('/users', async (request, reply) => {
    ...
    if (!decision.ok) {
      throw new ApiError(
        decision.code === 'NOT_ADMIN' ? 'AUTH_INVALID' : decision.code === 'EMAIL_TAKEN' ? 'AUTH_EMAIL_TAKEN' : 'VALIDATION',
    ...
    const user = await app.prisma.licenseUser.create({
      data: {
        licenseId: caller.licenseId, email, displayName: input.displayName,
        role: input.role, passwordHash: secret.hash, passwordSalt: secret.salt,
      },
    });
    return reply.status(201).send({ id: user.id, email: user.email, role: user.role, displayName: user.displayName });
  });
  ...
  app.delete('/users/:id', async (request, reply) => {
    ...
    const remainingAdmins = await app.prisma.licenseUser.count({
      where: { licenseId: target.licenseId, role: 'admin', active: true, id: { not: target.id } },
    });
    const decision = decideDeleteUser(caller, { id: target.id, role: target.role, active: target.active, licenseId: target.licenseId }, remainingAdmins);
    if (!decision.ok) {
      throw new ApiError(decision.code === 'LAST_ADMIN' ? 'AUTH_LAST_ADMIN' : 'AUTH_INVALID', decision.code === 'LAST_ADMIN' ? 409 : 403);
    }

    await app.prisma.licenseUser.update({ where: { id: target.id }, data: { active: false } });
